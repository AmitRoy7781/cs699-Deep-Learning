...
def command(
    ...
    *,
    ...
    rot_flip = "--rot-flip" if args.rot_flip else ""
    cmdargs = [
        source,
        seed,
        shuffle,
    ...
    if len(args.sbatch) > 0:
        ...
        submit = {"account": "-A", "partition": "--partition"}[submit]

    #
    heads = ["#!/bin/bash"]
    if len(args.sbatch) > 0:
        #
        heads.append("#SBATCH {:s} {:s}".format(submit, name))
    heads.append("#SBATCH --job-name={:s}".format(identifier))
    heads.append("#SBATCH --time=01:00:00 ")
    heads.append(
        "#SBATCH --output={:s}".format(
            os.path.join("sbatch", "{:s}.stdout.txt".format(identifier))
        ),
    )
    heads.append(
    ...
    for (i, argument) in enumerate(cmdargs):
        ...
        if len(argument) == 0:
            ...
            continue
        n_requires = (
            len(lines[-1]) + 1 + len(argument) + (2 if i < len(cmdargs) - 1 else 0)
        )
        if n_requires > n:
            #
