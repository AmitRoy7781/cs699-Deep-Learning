...
class GCN(torch.nn.Module):
    ...
    def __init__(self, num_inputs: int, num_outputs: int) -> None:
        ...
        self.bias = (
            torch.nn.Parameter(torch.zeros(self.num_outputs,))
        )
    ...
    def forward(
        ...
        h = node_embeds @ self.weight
        
        num_nodes = node_embeds.shape[0]
        adj = torch.sparse_coo_tensor(edge_indices, edge_weights, [num_nodes, num_nodes])
        h = adj.matmul(h)    # Sparse-dense matrix multiplication
        h = h + self.bias
